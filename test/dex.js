...
contract("Dex", (accounts) => {
  ...
  it("throw an error if token balance is too low when creating SELL order", async () => {
    ...
    let emerys = await Emerys.deployed();
    await truffleAssert.reverts(
      dex.createLimitOrder(1, web3.utils.fromUtf8("EMRS"), 10, 1)
    );
    await emerys.approve(dex.address, 500);
    await dex.addToken(web3.utils.fromUtf8("EMRS"), emerys.address, {
      from: accounts[0],
    });
    await dex.deposit(10, web3.utils.fromUtf8("EMRS"));
    await truffleAssert.passes(
      dex.createLimitOrder(1, web3.utils.fromUtf8("EMRS"), 10, 1)
    );
  });
  ...
  it("BUY order book should be ordered on price from highest to lowest", async () => {
    ...
    await dex.createLimitOrder(0, web3.utils.fromUtf8("EMRS"), 1, 200);
    await dex.createLimitOrder(0, web3.utils.fromUtf8("EMRS"), 1, 100);
    await dex.createLimitOrder(0, web3.utils.fromUtf8("EMRS"), 1, 300);
  ...
  it("SELL order book should be ordered on price from lowest to highest", async () => {
    ...
    let emerys = await Emerys.deployed();
    await emerys.approve(dex.address, 500);
    await dex.createLimitOrder(1, web3.utils.fromUtf8("EMRS"), 1, 200);
    await dex.createLimitOrder(1, web3.utils.fromUtf8("EMRS"), 1, 100);
    await dex.createLimitOrder(1, web3.utils.fromUtf8("EMRS"), 1, 300);

    let orderbook = await dex.getOrderBook(web3.utils.fromUtf8("EMRS"), 1);
    console.log(orderbook);
    for (let i = 0; i < orderbook.length - 1; i++) {
      assert(
        orderbook[i].price <= orderbook[i + 1].price,
