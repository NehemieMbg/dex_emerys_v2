...
contract("Dex", (accounts) => {
  ...
  it("Should throw an error when creating a sell market order without adequate token balance", async () => {
    ...
    assert.equal(balance.toNumber(), 0, "Initial EMRS balance is not 0");

    await truffleAssert.reverts(dex.createMarketOrder(1, emerys.address, 10));
  });
  ...
  it("Market orders can be submitted even if the order book is empty", async () => {
    ...
    let orderbook = await dex.getOrderBook(emerys.address, 0); //Get buy side orderbook
    assert(orderbook.length == 0, "Buy side Orderbook length is not 0");

    await truffleAssert.passes(dex.createMarketOrder(0, emerys.address, 10));
  });
  ...
  it("Market orders should not fill more limit orders than the market order amount", async () => {
    ...
    let orderbook = await dex.getOrderBook(emerys.address, 1); //Get sell side orderbook
    assert(
      orderbook.length == 0,
    ...
    await emerys.transfer(accounts[1], 150);
    await emerys.transfer(accounts[2], 150);
    await emerys.transfer(accounts[3], 150);

    //Approve DEX for accounts 1, 2, 3
    await emerys.approve(dex.address, 50, { from: accounts[1] });
    await emerys.approve(dex.address, 50, { from: accounts[2] });
    await emerys.approve(dex.address, 50, { from: accounts[3] });

    //Deposit LINK into DEX for accounts 1, 2, 3
    await dex.deposit(50, emerys.address, { from: accounts[1] });
    await dex.deposit(50, emerys.address, { from: accounts[2] });
    await dex.deposit(50, emerys.address, { from: accounts[3] });

    //Fill up the sell order book
    await dex.createLimitOrder(1, emerys.address, 5, 300, {
      from: accounts[1],
    });
    await dex.createLimitOrder(1, emerys.address, 5, 400, {
      from: accounts[2],
    });
    await dex.createLimitOrder(1, emerys.address, 5, 500, {
      from: accounts[3],
    });

    //Create market order that should fill 2/3 orders in the book
    await dex.createMarketOrder(0, emerys.address, 10);

    orderbook = await dex.getOrderBook(emerys.address, 1); //Get sell side orderbook
    assert(
      orderbook.length == 1,
  ...
  it("Market orders should be filled until the order book is empty", async () => {
    ...
    let balanceBefore = await dex.balances(accounts[0], emerys.address);

    //Create market order that could fill more than the entire order book (15 link)
    await dex.createMarketOrder(0, emerys.address, 50);

    //check buyer link balance after link purchase
    let balanceAfter = await dex.balances(accounts[0], emerys.address);
  ...
  it("The eth balance of the buyer should decrease with the filled amount", async () => {
    ...
    await emerys.approve(dex.address, 500, { from: accounts[1] });
    await dex.createLimitOrder(1, emerys.address, 1, 300, {
      from: accounts[1],
    });

    //Check buyer ETH balance before trade
    let balanceBefore = await dex.ethBalance(accounts[0]);
    await dex.createMarketOrder(0, emerys.address, 1);
    let balanceAfter = await dex.ethBalance(accounts[0]);
  ...
  it("The token balances of the limit order sellers should decrease with the filled amounts.", async () => {
    ...
    await emerys.approve(dex.address, 500, { from: accounts[2] });
    await dex.deposit(100, emerys.address, { from: accounts[2] });

    await dex.createLimitOrder(1, emerys.address, 1, 300, {
      from: accounts[1],
    });
    await dex.createLimitOrder(1, emerys.address, 1, 400, {
      from: accounts[2],
    });

    //Check sellers Emerys balances before trade
    let account1balanceBefore = await dex.balances(accounts[1], emerys.address);
    let account2balanceBefore = await dex.balances(accounts[2], emerys.address);

    //Account[0] created market order to buy up both sell orders
    await dex.createMarketOrder(0, emerys.address, 2);

    //Check sellers Emerys balances after trade
    let account1balanceAfter = await dex.balances(accounts[1], emerys.address);
    let account2balanceAfter = await dex.balances(accounts[2], emerys.address);
  ...
  it("Filled limit orders should be removed from the orderbook", async () => {
    ...
    await dex.deposit(50, emerys.address);
  ...
  it("Should throw an error when creating a buy market order without adequate ETH balance", async () => {
    ...
    let balance = await dex.balances(accounts[4], emerys.address);
    ...
    await truffleAssert.reverts(
      dex.createMarketOrder(0, emerys.address, 5, {
        from: accounts[4],
      })
    );
  });
